...
5. **OTA (Module Federation only)** — for federated apps, JS/Hermes updates can ship by replacing a remote chunk on the CDN, with no store submission. Plan skew guards, fallback, staged rollout, and rollback. See the plugin's `docs/OTA.md` (a plugin file, not a project file; online: https://github.com/DentVega/rn-repack-aidlc/blob/main/docs/OTA.md). (Single-bundle apps have no OTA path — suggest `/federate` to analyze whether any feature is worth carving into a remote.)
